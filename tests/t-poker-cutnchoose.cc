	...
	(std::istream& vtmf_str, size_t player)
{
	if ((pid[player] = fork()) < 0)
		perror("t-poker-cutnchoose (fork)");
	else
	{
		if (pid[player] == 0)
		{
			try
			{
				...
				start_clock();
				SchindelhauerTMCG *tmcg = new SchindelhauerTMCG(80, PLAYERS, 6);
					...
					if (i == player)
					{
						...
						for (size_t i2 = 0; i2 < PLAYERS; i2++)
						{
							...
							*P_out[i2] << s2 << std::endl;
							tmcg->TMCG_ProveStackEquality(s, s2, ss, false,
				...
				// release TMCG and VTMF instances
	...
	try
	{
		...
		for (size_t i = 0; i < PLAYERS; i++)
		{
			for (size_t j = 0; j < PLAYERS; j++)
			{
				if (pipe(pipefd[i][j]) < 0)
					perror("t-poker-cutnchoose (pipe)");
			}
		}
		...
		for (size_t i = 0; i < PLAYERS; i++)
		{
			...
			if (waitpid(pid[i], &wstatus, 0) != pid[i])
				perror("t-poker-cutnchoose (waitpid)");
			...
			for (size_t j = 0; j < PLAYERS; j++)
			{
				if ((close(pipefd[i][j][0]) < 0) || (close(pipefd[i][j][1]) < 0))
					perror("t-poker-cutnchoose (close)");
			}
		}
