	...
	else
	{
		if (pid[player] == 0)
		{
			...
			if (player == 0)
			{
				...
				{
					std::cout << "P_" << player << ": " <<
						"VRHE instance was not correctly generated!" << std::endl;
					exit(-1);
				}
				...
				{
					std::cout << "VRHE: Encryption scheme does not match!" << std::endl;
					exit(-1);
				}
				stop_clock();
				std::cout << "P_" << player << ": " << elapsed_time() << std::endl;
			...
			for (size_t i = 0; i < PLAYERS; i++)
			{
				...
				if (i == player)
				{
					...
					tmcg->TMCG_ProveStackEquality_Hoogh_noninteractive(s, s2,
						ss, vtmf, vrhe, lej);
					...
					*P_in[i] >> s2;
					...
					if (!tmcg->TMCG_VerifyStackEquality_Hoogh_noninteractive(s, s2,
						vtmf, vrhe, *P_in[i]))
					{
