	...
	(mpz_ptr m, size_t &i_out)
{
	...
	try
	{
		...
		{
			size_t l; // receive a message from an arbitrary party $P_l$
...
std::cerr << "RBC: received message from " << l << " with tag " << message[0] << "." << message[1] << "." << message[2] << std::endl; 
				...
				if (mpz_cmp_ui(message[2], 1) < 0) // FIXME: check against global message counter
				{
...
std::cerr << "RPC: more ready" << std::endl;
					...
					else if ((*rit).second == ((2 * t) + 1))
					{
						...
						{
std::cerr << "RPC: answer needed" << std::endl;
							...
							do
							{
								...
								if (aiou->Receive(message3, l2))
								{
									...
									{
// TODO: buffer message for next deliver
									}
									else if (!answer[l2].count(tag_string))
									{
